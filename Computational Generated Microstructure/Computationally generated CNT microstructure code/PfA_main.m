...
RVE = [];
RVE.Vf = 0.007;                               %Volume fraction
RVE.size = [10,10,10];                       %[Lx,Ly,Lz] mimcrometer
...
RVE.dcut = 1.0e-3;                           %cut off tunnelling distance %micrometer
...
RVE.nBin = 3;                                %number of grids/bins in each direction


%% form CNT network
[ Ptrue1, Center1, Length,~,~,~] = A_GenerateHardCore(RVE);    %Generate the CNT
pipi=Ptrue1';
n=size(pipi,2);
Linit = RVE.size;
...
[dCNT, ja] = A_CNT_contact( PP', Pcenter', Plength, RVE); %Find all tunneling distances and the junctions on CNTs
[Gn, perco ] = A_ResNetwork_Soft(dCNT, ja, PP', RVE); %find the network resistance matrix
