function [ G, perco ] = A_ResNetwork_Soft_leafs_tun(dCNT, ja, PP, RVE)
...
%     end
    if ismember(conCNT(i),indI)                         % ISRC node
        ...
        ps = [ps (0.25*pi*RVE.sigma*RVE.D^2)*1e-6/LCNT];
    end
    if ismember(conCNT(i),indG)                         % GND node
        ...
        [E,indp] = sort(abs(da-(conP(i,q)-0.1)));         % find distance (substract 0.1 added in distance between segment)
        LCNT = E(1) + (E(1)<1e-6)*0.1;                  % if length close to zero, add 0.1 to its value
        qs = [qs (0.25*pi*RVE.sigma*RVE.D^2)*1e-6/LCNT];
    end
end
...
GMat(p,1+conN) = ps;                                     % The CNTs connected to IsrcE are connected to ISRC node by RK
GMat(1+conN,p) = ps;
GMat(q,2+conN) = qs;                                     % The CNTs connected to GndE are connected to GndE node by RK in THIS program
GMat(2+conN,q) = qs;

GMat = -GMat + diag(sum(GMat));                         % conductance adjacent matrix
G = GMat(1:1+conN, 1:1+conN);    
end
