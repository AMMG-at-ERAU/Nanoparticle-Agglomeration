function [ dCNT, ja ] = Hybrid_contact_holes( PP, Pcenter, Plength, Resistor_GNP, Resistor_Center_GNP, Center_GNP, rad_GNP, R, Radius_holes, RVE)
        ...
        if ~isempty(find(temp, 1))
            for j=1:length(find(temp))
                ...
                if isnan(P1(1)) ~= 1
                    ...
                    dist1 = Dist_point2holes(R,[P1(1),0,P1(2)]); %distance with all holes center
                    dist2 = Dist_point2holes(R,[P2(1),0,P2(2)]);
                    dist1 = (dist1<=rad_hole); % have values 1 everywhere it is less than radius
                    dist2 = (dist2<=rad_hole);
                    dist = sum(dist1)+sum(dist2);
                    if (dist>=1)
                        dt = 2*RVE.dcut + 2e-6; %Make it bigger then tunnelling distance so it acts as if not in percolated network
                    end
    ...
    for i=2:Ngnp
        ...
        for j=1:i-1
            ...
            if (~isempty(nb) & all(pnew(1,:)>=0) & all(pnew(1,:)<=RVE.size(1)) & all(pnew(2,:)>=0) & all(pnew(1,:)<=RVE.size(3))) %make sure that the points are inside the RVE
                [dt, PTi, PTa] = dist_multiple_points_ellipse_holes( pnew, P2, ae, be, alpha,d_tun);
                P1c = [Resistor_Center_GNP(i,1);Resistor_Center_GNP(i,3)];%fictitious center of i ellipse
                dti = sqrt((P1c(1) - p1(1))^2 + (P1c(2) - p1(2))^2) + 1e-5;% (i ellipse)I have added 0.1 so that the value won't be zer0 when the intersection is p1 or p3. I will subtract it
%                 dta = sqrt((P2c(1) - p3(1))^2 + (P2(2) - p3(2))^2) + 1e-5;% (j ellipse) every time before using the value
                dta = sqrt((P2c(1) - p3(1))^2 + (P2c(2) - p3(2))^2) + 1e-5;% (j ellipse) every time before using the value
%                 dist1 = Dist_point2holes(R,[PTi(1),0,PTi(2)]); %distance with all holes center
%                 dist2 = Dist_point2holes(R,[PTa(1),0,PTa(2)]);
                dist1 = Dist_point2holes(R,[P1c(1),0,P1c(2)]); %distance with all holes center
                dist2 = Dist_point2holes(R,[P2c(1),0,P2c(2)]);
                dist1 = (dist1<=rad_hole); % have values 1 everywhere it is less than radius
                dist2 = (dist2<=rad_hole);
                dist = sum(dist1)+sum(dist2);
                if (dist>=1)
