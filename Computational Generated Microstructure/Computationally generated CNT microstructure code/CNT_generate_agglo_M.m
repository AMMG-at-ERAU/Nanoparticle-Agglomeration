function [  Ptrue, Center, Length, Angle, Ptrue1, Center1, Length1 ] = CNT_generate_agglo_M( RVE)
...
while Vpr1<RVE.Vf*Lx*Lz*RVE.Vf_agg %generation of agglomerated CNTs

    % Center points (cx,cy,cz) is random
    cx = Center(1,Nrand(i)) + agg_rad*x_a(i);
    cy = 0;
    cz = Center(3,Nrand(i)) + agg_rad*y_a(i);
    if cx>Lx || cx<0 % if the center is outside of RVE, apply PBC
        cx = (cx>Lx)*(cx - Lx) + (cx<0)*(cx + Lx); 
    end
    if cz>Lz || cz<0 % if the center is outside of RVE, apply PBC
        cz = (cz>Lz)*(cz - Lz) + (cz<0)*(cz + Lz); 
    end
    r = rand(2,1);
    phi = 2*pi*r(1,:);
    ...
    B = find(a==Nrand(i), 1); %See if this non agglo CNTs have been used before as agglo center
    if isempty(B) % if not
            plot(Center(1,Nrand(i))+xp,Center(3,Nrand(i))+yp,'r','LineWidth',2);
            hold on
            a = [a Nrand(i)];
            Vp = Vp + RVE.D * Length(Nrand(i)); % add non-agglo CNTs to volume fraction of agglo
            Vpr = Vpr - RVE.D * Length(Nrand(i)); % substract non-agglo CNTs from volume fraction of non agglo
    end
    x1 = cx-0.5*li.*(cos(phi));
    y1 = 0;
    z1 = cz-0.5*li.*(sin(phi));
...
while Vpr<RVE.Vf*Lx*Lz*(1-RVE.Vf_agg) %replace the non agglo CNT used as agglo center
    r = rand(4,1);
    % Center points (cx,cy,cz) is random
    cx = Lx*r(1,:);
    cy = 0;
