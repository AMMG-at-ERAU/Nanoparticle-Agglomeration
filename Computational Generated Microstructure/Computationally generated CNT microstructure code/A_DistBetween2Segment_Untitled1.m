...
function [distance, dti, dta] = A_DistBetween2Segment_Untitled1(p1, p2, p3, p4,dvdw,Dia)
...
% dta           distance between intersection on p3p4 and p3
    ...
    dti = sqrt(sum((p1-v1).*(p1-v1)))+0.5; %I have added 0.1 so that the value won't be zer0 when the intersection is p1 or p3. I will subtract it
    dta = sqrt(sum((p3-v2).*(p3-v2)))+0.5; %every time before using the value
    if distance <1e-8
        distance = dvdw;
    end
%  if distance <=1e-6
