function [Nt, Nt1, Nt2, G, perco ] = ResNetwork_Hybrid_Soft_leafs_notouch_Nj_Nj1( Ncnt, dCNT, ja, PP, RVE)
...
Nj = length(find(dCNT));
NjCNT = length(find(dCNT(1:Ncnt,1:Ncnt)));
NjGNP = length(find(dCNT(Ncnt+1:end,Ncnt+1:end)));
Nt = [NjCNT NjGNP Nj];
...
for i=1:length(ind1)
    [root1(i),ptr] = find_root(ind1(i),ptr); %find roots of CNTs on that edge
end
for i=1:length(ind2)
    [root2(i),ptr] = find_root(ind2(i),ptr); %find roots of CNTs on that second edge
end
...
p = 0;
for i = 1:length(root)
    R = find_leafs(root(i),ptr); %find all leafs of that root
...
conCNT = [indI innCNT indG];                            % conductive fillers
conN = length(conCNT);                                  % conductive fillers index
conGNP = find(conCNT>Ncnt);                             % conductive GNP index
%% Calculation of Rt and Rint

Nj = length(find(DCNT(conCNT,conCNT)));
NjCNT = length(find(DCNT(conCNT<=Ncnt,conCNT<=Ncnt)));
NjGNP = length(find(DCNT(conCNT>Ncnt,conCNT>Ncnt)));
Nt1 = 0.5*[NjCNT NjGNP Nj];

Nj = length(find(DCNT(innCNT,innCNT)));
NjCNT = length(find(DCNT(innCNT<=Ncnt,innCNT<=Ncnt)));
NjGNP = length(find(DCNT(innCNT>Ncnt,innCNT>Ncnt)));
Nt2 = 0.5*[NjCNT NjGNP Nj];
...
for i=1:conN                                            % Inner nodes, Gjunction
    [~,q,d] = find(conJun(i,i+1:conN));
    for j=1:length(d)
        Gnode(IndN(i,i+q(j)),IndN(i+q(j),i)) = 1./ResTun1( RVE, d(j) );  % find tunnelling conductance
    end
end
...
for i=1:conN                                            % Inner nodes, Gcnt
    ...
    for j=2:length(d)
        ...
        if LCNT<1e-5 %close to zero
            LCNT = 1e-5;                                 % LCNT should be > 0
        end
        if b 
            Gnode(IndN(i,q(indp(j-1))),IndN(i,q(indp(j)))) = pi*RVE.be/(2*RVE.Re*LCNT);  % GNP
        else
    ...
    if ismember(conCNT(i),indI)                         % ISRC node
        ...
        [E,indp] = sort(abs(da-(conP(i,q)-1e-5)));       % find distance (substract 1e-5 added in distance between segment)
        LCNT = E(1) + (E(1)<1e-6)*1e-5;                  % if length close to zero, add 1e-5 to its value
        if b 
            Gnode(IndN(i,q(indp(1))),node) = pi*RVE.be/(2*RVE.Re*LCNT);  % GNP
        else
            Gnode(IndN(i,q(indp(1))),node) = (0.25*pi*RVE.sigma*RVE.D^2)*1e-6/LCNT;  % CNT
        end
    end
    if ismember(conCNT(i),indG)                         % GND node
        ...
        LCNT = E(1) + (E(1)<1e-6)*1e-5;                  % if length close to zero, add 1e-5 to its value
        if b 
            Gnode(IndN(i,q(indp(1))),node+1) = pi*RVE.be/(2*RVE.Re*LCNT);  % GNP
        else
            Gnode(IndN(i,q(indp(1))),node+1) = (0.25*pi*RVE.sigma*RVE.D^2)*1e-6/LCNT;  % CNT
        end
    end
end
