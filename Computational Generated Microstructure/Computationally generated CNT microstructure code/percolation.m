...
%% form CNT network
[ ~,~,~,Ptrue, Center, Plength] = A_HardCore_multiple_Grids_small_boxp(RVE);    %Generate the CNT
PP=Ptrue';
n=size(PP,2);
%Linit = RVE.size;
...
%    Res1 = xn(length(Gn));
  ...
  % disp(['// Resistance = ',num2str(Res1),' S/m']);
   ...
   conductivity = 1/Gtotal1;
   toc

end
Total(ii,:) = [conductivity junct fLAG];
%end
...
tstop = toc(tstart)
save('CondCNT3D_0.120_2000.mat','Total')
