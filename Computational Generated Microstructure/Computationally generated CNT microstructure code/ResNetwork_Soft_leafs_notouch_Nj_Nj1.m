function [Rf, Nj, Nj1, Nj2, G, perco ] = ResNetwork_Soft_leafs_notouch_Nj_Nj1(dCNT, ja, PP, RVE)
...
Rf = NaN;
%Rf = ResTun1( RVE, (mean(dCNT(dCNT>0))) );
...
Rf = ResTun1( RVE, mean((conJun(conJun>0))) );
...
% G = Gnode(1:length(Gnode)-1,1:length(Gnode)-1);  
Gnode = diag(sum(Gnode)) - Gnode;                       % conductance adjacent matrix
