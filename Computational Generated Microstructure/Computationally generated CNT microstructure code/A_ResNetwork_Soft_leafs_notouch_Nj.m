function [Nj, G, perco ] = A_ResNetwork_Soft_leafs_notouch_Nj(dCNT, ja, PP, RVE)
...
elseif RVE.Dir=='y'
    ya = PP(1:2:end,2);
    yb = PP(2:2:end,2);
    ind1 = find((abs(ya)<=1e-9)|(abs(yb)<=1e-9));          % y cross the 1st edge, connected to ISRC
    ind2 = find(abs(ya-RVE.size(2))<=1e-9|abs(yb-RVE.size(2))<=1e-9);            % y cross the 2nd edge, connected to GND
else
    xa = PP(1:2:end,1);
    xb = PP(2:2:end,1);
    ind1 = find((abs(xa)<=1e-9)|(abs(xb)<=1e-9));         % x cross the 1st edge, connected to ISRC
    ind2 = find(abs(xa-RVE.size(1))<=1e-9|abs(xb-RVE.size(1))<=1e-9);           % x cross the 2nd edge, connected to GND
end
...
if ~isempty(root)
    %disp('// CNT network formed...');       %(this is changed for display)
...
for i=1:conN                                            % Inner nodes, Gcnt
    ...
    if ismember(conCNT(i),indI)                         % ISRC node
        ...
        elseif RVE.Dir=='y'
        P2 = P1(abs(P1(:,2))<=1e-9,:);           % find end point on ISRC
        else
        P2 = P1(abs(P1(:,1))<=1e-9,:);             % find end point on ISRC
        end
    ...
    if ismember(conCNT(i),indG)                         % GND node
        ...
        elseif RVE.Dir=='y'
        P2 = P1(abs(P1(:,2)-RVE.size(2))<=1e-9,:);   % find end point on GND
        else
        P2 = P1(abs(P1(:,1)-RVE.size(1))<=1e-9,:);   % find end point on GND
        end
